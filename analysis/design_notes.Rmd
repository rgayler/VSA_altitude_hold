...
In a mathematical model the the quantities are unscaled, they are just
...
metres per second or [furlongs per
...
### Relation of VSA and multicopter simulation components
...
### Structure of the VSA data flow graph implementation
...
-   Some initial exploratory VSA work can be done as standalone analyses
    ...
    -   Each row of the data frame corresponds to a discrete time step,
        $t = 1:n$. ($n$ will be fixed for any run of the system, so the
        storage in the data frame can be pre-allocated if that is needed
        for efficiency.)

    -   Each column of the data frame corresponds to an input variable,
        an internal state variable of the altitude controller, or an
        output variable.
    ...
    -   At each time step $t$ a state update function is applied to the
        ...
        -   The details of that accumulation of rows into a data frame
            have not yet been decided. There are some issues around
            computational efficiency. It may be best to generate each
            row as a one-row data frame, accumulate those one-row data
            frames in a list, then bulk-convert the list to a single
            data frame (see
...
-   The edge between a reference node and the corresponding node is
    ...
    calculation and is not actually part of the datat flow diagram.
